...
def ejecutar():
    ...
    if len(sys.argv) == 1:
        ...
        print("Uso: python udp_cliente5_mejorado.py <ipServidor> <puertoServidor>")
        sys.exit(1)
    ...
    # Mejora Implementada (uso de connect())
    try:
        cliente.connect(direccionServidor)
        print(f"Socket 'conectado' a {servidor}:{puerto} para filtrar respuestas")
    except Exception as e:
        print(f"Error al intentar 'conectar' el socket: {e}")
        sys.exit(1)
    ...
    while True:
        try:
            ...
            while not recibidoACK and actualTimeout <= maxTimeout:
                ...
                # Cambio01: Utilización de send() en lugar de sendto() debido al connect()
                cliente.send(mensajeCompleto.encode('utf-8'))
                ...
                try: 
                    ...
                    # Cambio02: Utilización de recv() en lugar de recvfrom() debido al connect()
                    respuestaBytes= cliente.recv(1024)
