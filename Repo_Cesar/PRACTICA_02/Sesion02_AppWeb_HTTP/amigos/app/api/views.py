...
def list_amigos():
     ...
     """
     # Consultamos todos los amigos
     todos_los_amigos = Amigo.query.all()

     # Convertimos la lista de objetos Amigo en una lista de diccionarios
     lista_amigos_dict = []
     for amigo in todos_los_amigos:
         amigodict = {'id': amigo.id, 'name': amigo.name,
                      'lati': amigo.lati, 'longi': amigo.longi}
         lista_amigos_dict.append(amigodict)

     # Devolvemos la lista en formato JSON
     return jsonify(lista_amigos_dict)
